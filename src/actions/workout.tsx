...
async function addWorkout(userId: string, workout: NewWorkout): Promise<{ title: string; message: string }> {
    ...
    try {
        ...
        if (user.workouts) {
            // Fetch the last workout (now with the real _id generated by MongoDB)
            const savedWorkout = user.workouts[user.workouts.length - 1];

            logger.info(`Workout added: ${JSON.stringify(savedWorkout)}`);
        }
...
async function updateWorkout(userId: string, workoutId: string, workoutUpdateData: patchReqDataType): Promise<{ title: string; message: string }> {
    ...
    try {
        ...
        // Perform the update for only fields provided in the request
        await user?.workouts?.id(workoutId).updateOne(
